// Color Dash — clean fixed edition
// Load after DOM. This file implements:
// - main menu (Play -> levels) with "Customize your cube" inside the Play menu
// - floating level editor button and editor with live preview
// - prevents "You crashed" on page load (game paused until a level is started)
// - audio is created/resumed only after a user gesture (prevents autoplay warnings)
// - touch listeners with appropriate passive options when needed
...
(function () {
  ...
  function init(){
    ...
    // Events
    ui.btnPlay.addEventListener('click', ()=> openMenu('play'));
    ui.btnSkins.addEventListener('click', ()=> openMenu('skins'));
    ui.btnEditor.addEventListener('click', ()=> openEditor());
    ui.menuClose.addEventListener('click', ()=> closeMenu());
    ui.goRestart.addEventListener('click', ()=> { restartRun(); hideGameOver(); });
    ui.goMenu.addEventListener('click', ()=> { hideGameOver(); openMenu('play'); });
    ui.floatingEditor.addEventListener('click', ()=> openEditor());
    ui.btnMusic.addEventListener('click', ()=> { isMusicOn = !isMusicOn; ui.btnMusic.textContent = 'Music: ' + (isMusicOn ? 'On' : 'Off'); if (isMusicOn) startMusic(); else stopMusic(); });
    ui.btnFull.addEventListener('click', ()=> { if (!document.fullscreenElement) document.documentElement.requestFullscreen().catch(()=>{}); else document.exitFullscreen(); });

    // input: allow keyboard, mouse, touch. Touch handlers that preventDefault use passive:false
    ...
    // small audio resume on first user gesture to avoid autoplay warnings
    ...
    // Show the Play menu at first load if you want — comment out to show blank page
    // openMenu('play');
  }
  ...
  // ---------- Public small set of functions for editor/game wiring ----------
  function startLevelById(id){ const lvl = levels.find(l => l.id === id); if (lvl) startLevel(lvl); }

  // ---------- Expose some functions for debug ----------
  window.CD = {
    startLevelById, levels, saveLevels: ()=> localStorage.setItem(LS_KEY_LEVELS, JSON.stringify(levels)), setSkin: saveSelectedSkin, gameState
  };
